...
class haarcascade:
    ...
    def scale_down(self):
        ...
        cv.Resize(self.frame, dst, interpolation=cv.CV_INTER_LINEAR)
    ...
    def detect_objects(self, frame):
        ...
        if self.frame_index % self.getDownsamplingFactor() == 0:
            detectedObjects = cv.HaarDetectObjects(self.frame, self.cascade, STORAGE, self.video_quality_factor, self.confidence_threshold)
            if detectedObjects:
