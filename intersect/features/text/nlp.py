...
class driver:
  ...
  def select_actions(self, actionList):
    ...
    for action in actionList:
      ...
      self.actions[action] = self.get_new_action_processor(action)

    print "------------------------"

  def get_action_processor(self, action_name):
    action = None
    try:
      action = self.actions[action_name]
    except:
      action = None
      print "The action '" + str(action_name) + "' could not be fetched"

    return action

  def get_new_action_processor(self, action):
...
class NlpCLI:
  ...
  def display_available_methods(self):
    ...
    print ""
    print "Available methods for '" + str(self.action_name) + "'"
    print "-------------------------------------"
